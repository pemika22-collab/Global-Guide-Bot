...
Keep responses conversational and helpful. Focus on understanding their travel desires."""
    ...
    async def process(
        ...
        if message_lower in ['cancel', 'reset', 'start over', 'restart', 'clear', 'new search']:
...
Please confirm to proceed with booking. Reply 'yes' to confirm or 'no' to cancel."""
            ...
            self.conversation_context.clear()  # Clear old flags
...
- customer_name: Tourist's name (e.g., "Sarah Johnson", "My name is Adam")
- date: Tour date
- number_of_people: Number of people
- tour_type: Type of tour
...
{{
    ...
    async def _handle_guide_search(
        ...
        if not extracted.get('date'):
            ...
            # Preserve image analysis context
            new_context = {
                "awaiting_date": True,
                "search_criteria": extracted
            }
            # Keep suggested location and interests from image analysis
            if self.conversation_context.get('suggested_location'):
                new_context['suggested_location'] = self.conversation_context['suggested_location']
            if self.conversation_context.get('suggested_interests'):
                new_context['suggested_interests'] = self.conversation_context['suggested_interests']
            # Keep last_interaction_type so we know this came from image analysis
            if self.conversation_context.get('last_interaction_type'):
                new_context['last_interaction_type'] = self.conversation_context['last_interaction_type']
        ...
        try:
            ...
            if date_str:
                ...
                today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
                if requested_date.date() < today.date():
                    print(f"⚠️ Date {date_str} is in the past")
                    return {
                        "message": f"The date {date_str} is in the past. Please provide a future date for your tour.",
                        "intent": intent,
                        "reasoning": self.reasoning_steps,
                        "context": {
                            "awaiting_date": True,
        ...
        # Check if we have location - if not, check context or ask for it
        if not extracted.get('location'):
            # Check if we have a suggested location from image analysis
            if (self.conversation_context.get('last_interaction_type') == 'image_analysis' and 
                self.conversation_context.get('suggested_location')):
                
                suggested_location = self.conversation_context['suggested_location']
                print(f"📍 Using location from image analysis: {suggested_location}")
                extracted['location'] = suggested_location
                
                # Also use suggested interests if available
                if self.conversation_context.get('suggested_interests'):
                    suggested_interests = self.conversation_context['suggested_interests']
                    print(f"🎯 Using interests from image analysis: {suggested_interests}")
                    extracted['interests'] = suggested_interests
    ...
    "reasoning": "brief explanation"
        ...
        if not guide_id and guide_name:
            ...
            try:
                ...
                table = dynamodb.Table('global-guide-bot-dev-guides')
                response = table.scan()
...
Keep it exactly in this format."""
        ...
        if needs_booking and guide_analysis.get('intent') == 'BOOKING_CONFIRMATION':
...
1. GUIDE_SEARCH: User wants to find/search for guides
...
4. BOOKING_CONFIRMATION: User confirms/agrees to booking
   ...
   • CONTEXT: Only when previous message offered booking/guide services
